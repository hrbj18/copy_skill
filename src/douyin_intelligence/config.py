...
def load_config(path: str | Path | None = None) -> dict[str, Any]:
    ...
    material_replication = jobs.get("material_replication")
    if not isinstance(material_replication, dict):
        raise ConfigurationError("jobs.material_replication 必须是对象")
    if not 1 <= int(material_replication.get("min_pool_size") or 0) <= int(material_replication.get("default_pool_size") or 0) <= int(material_replication.get("max_pool_size") or 0):
        raise ConfigurationError("素材复刻候选池必须满足 min_pool_size ≤ default_pool_size ≤ max_pool_size")
    if not 3 <= int(material_replication.get("min_keywords") or 0) <= int(material_replication.get("max_keywords") or 0) <= 10:
        raise ConfigurationError("素材复刻关键词数量必须满足 3 ≤ min_keywords ≤ max_keywords ≤ 10")
    if int(material_replication.get("theme_max_chars") or 0) <= 0:
        raise ConfigurationError("素材复刻主题长度上限必须大于 0")
    if int(material_replication.get("max_path_chars") or 0) < 100:
        raise ConfigurationError("素材复刻交付路径长度上限过小")
    budget = material_replication.get("budget")
    if not isinstance(budget, dict):
        raise ConfigurationError("jobs.material_replication.budget 必须是对象")
    for phase in ("collection", "download", "asr", "face", "clip", "total"):
        soft = int(budget.get(f"{phase}_seconds") or 0)
        hard = int(budget.get(f"{phase}_hard_seconds") or 0)
        if soft <= 0 or hard < soft:
            raise ConfigurationError(f"素材复刻 {phase} 预算必须满足 0 < 软上限 ≤ 硬上限")
    script_replica = material_replication.get("script_replica")
    if not isinstance(script_replica, dict) or int(script_replica.get("target_count") or 0) != 1:
        raise ConfigurationError("素材复刻脚本复刻视频目标数量必须为 1")
    if int(script_replica.get("min_chars") or 0) <= 0 or float(script_replica.get("min_chars_per_second") or 0) <= 0:
        raise ConfigurationError("素材复刻脚本复刻视频口播阈值无效")
    if not 0 < float(script_replica.get("min_seconds") or 0) < float(script_replica.get("max_seconds") or 0):
        raise ConfigurationError("素材复刻脚本复刻视频时长区间无效")
    if not 0 < float(script_replica.get("top_ratio") or 0) <= 1 or int(script_replica.get("min_top") or 0) < 1:
        raise ConfigurationError("素材复刻脚本复刻视频热度门槛无效")
    ...
    if int(clips.get("max_per_video") or 0) < 1 or int(clips.get("max_total") or 0) < 1:
        raise ConfigurationError("素材复刻片段数量上限无效")
    face = material_replication.get("face")
    if not isinstance(face, dict):
        raise ConfigurationError("jobs.material_replication.face 必须是对象")
    if not 0 <= float(face.get("free_max_ratio") or 0) <= float(face.get("low_max_ratio") or 0) <= 1:
        raise ConfigurationError("素材复刻人脸比例阈值必须满足 free_max_ratio ≤ low_max_ratio")
    face_priority = face.get("backend_priority")
    if not isinstance(face_priority, list) or not face_priority or not set(map(str, face_priority)) <= {"opencv_yunet", "opencv_dnn"}:
        raise ConfigurationError("素材复刻人脸后端优先级必须是 opencv_yunet/opencv_dnn")
    for key in ("output_root", "temp_root", "cache_root", "media_root"):
        value = Path(str(material_replication.get(key) or ""))
        if not str(value) or value.is_absolute() or ".." in value.parts:
            raise ConfigurationError(f"jobs.material_replication.{key} 必须是项目内相对路径")
    model_root = Path(str(face.get("model_root") or ""))
    if not str(model_root) or model_root.is_absolute() or ".." in model_root.parts:
        raise ConfigurationError("jobs.material_replication.face.model_root 必须是项目内相对路径")
    prefilter = material_replication.get("prefilter")
    if prefilter is not None:
        if not isinstance(prefilter, dict):
            raise ConfigurationError("jobs.material_replication.prefilter 必须是对象")
        if not 0 <= float(prefilter.get("heat_gate_percentile") or 0) <= 1:
            raise ConfigurationError("素材复刻下载前预筛热度分位必须落在 0~1")
        prefilter_min = float(prefilter.get("min_seconds") or 0)
        prefilter_max = float(prefilter.get("max_seconds") or 0)
        if prefilter_min < 0 or prefilter_max < 0 or (prefilter_max > 0 and prefilter_min > prefilter_max):
            raise ConfigurationError("素材复刻下载前预筛时长区间无效")
    download_budget = material_replication.get("download_budget")
    if download_budget is not None:
        if not isinstance(download_budget, dict):
            raise ConfigurationError("jobs.material_replication.download_budget 必须是对象")
        # The three caps only mean something when the budget is *on*.  A section
        # that is absent, or present but disabled, is a no-op: the user is
        # switching the budget off, so the caps are not validated at all.
        # Otherwise the most natural "turn it off" spelling -- ``{"enabled":
        # false}`` -- would be rejected, and there would be no way to disable
        # the budget from the only surface the user actually edits (the config
        # file).
        if bool(download_budget.get("enabled", False)):
            # ``0`` is rejected on purpose.  A literal 0 is ambiguous ("download
            # nothing" vs "no cap"), and the runtime defaults it to *unlimited*,
            # so a user writing ``max_count: 0`` to mean "本期不下载" would instead
            # get a full download -- the exact opposite of the intent.  The
            # "use enabled=false" hint is only true here, where enabled is true.
            for key in ("max_count", "max_bytes", "max_item_bytes"):
                if int(download_budget.get(key) or 0) < 1:
                    raise ConfigurationError(
                        f"素材复刻下载预算 {key} 必须 ≥ 1（当前 enabled=true；如需关闭预算请改为 download_budget.enabled=false）"
                    )
    validation = material_replication.get("validation")
    # Like ``download_budget``: an absent or ``None`` section is a no-op (the
    # layer is off), so it is not validated.  Only a present object is checked,
    # and only its numeric knobs, mirroring this file's loose style elsewhere.
    if validation is not None:
        if not isinstance(validation, dict):
            raise ConfigurationError("jobs.material_replication.validation 必须是对象")
        tolerance = float(validation.get("duration_tolerance") or 0)
