...
def download_video(url: str, destination: Path, config: dict[str, Any]) -> None:
    destination.parent.mkdir(parents=True, exist_ok=True)
    if destination.is_file() and destination.stat().st_size > 1024:
        return
    settings = config["materials"]
    error: Exception | None = None
    for attempt in range(int(settings.get("download_retries") or 3)):
        temporary = destination.with_suffix(destination.suffix + ".part")
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0", "Referer": "https://www.douyin.com/"})
            size = 0
            with urllib.request.urlopen(request, timeout=int(settings.get("download_timeout_seconds") or 180)) as response, temporary.open("wb") as stream:
                while chunk := response.read(1024 * 1024):
                    size += len(chunk)
                    if size > int(settings.get("max_video_bytes") or 524288000):
                        raise ValueError("视频超过允许的最大体积")
                    stream.write(chunk)
            if size <= 1024:
            ...
            if attempt + 1 < int(settings.get("download_retries") or 3):
