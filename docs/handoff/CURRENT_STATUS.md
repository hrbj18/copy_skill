...
- Shipped config enables `theme_material_profiles`/`theme_profile_map`, `sources: ["bilibili"]`,
  ...
  `max_age_days=90`, the per-theme `theme_keywords`/`theme_subject_terms` (`93c76da`) and the frozen
...
- Live proof 2026-09-18: `9.18日本AI篡改历史观复刻视频` -> 9 clips (2 main + 7 support), folder
  ...
  faces.
- Command: `material-replication run --theme <T> --pool-size <N> --business-date <YYYY-MM-DD>`. Driven
  by a host agent session, disable that host's bulk-delete guard first (host-level; `src/` does not read
  it), else the motion stage is killed mid-batch.
...
- Bounded OCR/ASR enrichment; dated Markdown/JSON packages, material manifests, `00-素材目录.json`,
  OpenMontage snapshots; bounded public-web leads plus visual references for a selected script.
...
- Autonomous daily-news discovery is not a reliable product (2026-09-03: zero strict cards).
...
2. Subject terms do not separate exact brand/model terms from category aliases, so a bare category word
   admits a competitor clip; fixing it forces a batch re-run.
...
- `third_party/MediaCrawler` is declared in `.gitmodules` but **not** recorded as a gitlink, so
  `git submodule update --init` does not fetch it.
...
- Full suite 2026-09-18: 1109 tests, 0 failures.
