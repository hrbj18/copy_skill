...
## 十、本轮实测结果（收口，2026-09-14 晚）

### 10.1 一句话结论

**验收标准 74 项全过（0 失败），7 期全部 EXIT=0**；对比旧交付的 **27 失败 / 67 项**。
开工基线 744 passed → 收工 **820 passed / 0 failed**；`git ls-files --deleted` 为空；冻结 4 期目录未被触碰。

### 10.2 测试类判据（A1 / A5 / A6）

| 判据 | 结果 | 证据 |
|---|---|---|
| A1 全量 0 failed | ✅ | EXIT=0，点行计数 820，`FAILED/ERROR` 计数 0（本仓不打印 summary 行） |
| A5 离线池回归 | ✅ | `.tmp/predict_gate_0914.py` 11 池复算：**2 期填洞、9 期逐字节不变** |
| A6 测试不得与生产配置耦合 | ✅ | seam 延伸到 `material_replica` 块；删除"断言线上配置关着"的重言式用例 |

### 10.3 交付类判据（7 期，B/C/D 全绿）

| 期 | 原片 | 体积 | 标题命中 | 视觉（hit/miss） | 交付片龄最旧 | 闸门准入 |
|---|---|---|---|---|---|---|
| Microduck 机械鸭机器人 | **9** | 70.4 MiB | **9/9 = 100%** | 5 / 4 | 9d | 44/45 |
| iRobot Roomba 875 | 5 | 99.3 MiB | 5/5 = 100% | 3 / 2 | 8d | 52/52 |
| 充电宝3C认证新规 | 6 | 85.1 MiB | 6/6 = 100% | 4 / 2 | 3d | 31/32 |
| 内存涨价 最贵装机季 | **2** | **5.1 MiB** | 2/2 = 100% | 0 / 2（无结论） | 15d | 12/31 |
| 显卡涨价 RTX5090 | 7 | **65.7 MiB** | 7/7 = 100% | 3 / 4 | 0d | 16/73 |
| 苹果 iPhone Duo 折叠屏 | 8 | 94.7 MiB | 8/8 = 100% | 4 / 4 | 0d | 70/70 |
| DeepSeek V4.1 Flash | 7 | 97.6 MiB | 7/7 = 100% | 7 / 0 | 4d | 31/31 |

- **B7**（已知跑偏样本）✅：Roomba 期不再出现 `1X Neo`/`Unitree G1`，Microduck 期不再出现 `Unitree G1`。历史同期交付的是人形机器人，本轮 9/9 全是机械鸭/HuggingFace 实拍。
- **C1** 跨期重复 0 个；**C2** 全部交付片龄在 90 天内；**C3** 清单已含 `title/bytes/published_at/source/hit_ratio/visual_verdict`；**D2** 无任何人工确认环节。

### 10.4 两个残余（**已如实标注，不是缺陷漏报**）

1. **内存涨价 最贵装机季 = 2 条 / 5.1 MiB**。剔除阶段直方图（池 26）：`relevance 13 / stale 6 / face 4 / speech 1`。
   ⇒ 瓶颈是**池内本身缺该题材**（26 条里 13 条标题不含"内存涨价/最贵装机季"），**不是时效窗**：
   即便把窗口放到无限大，多出来的 6 条里另有 4 条会被人脸闸门拒掉。⇒ **无需改配置**。
2. **显卡涨价 RTX5090 = 7 条 / 65.7 MiB**（差 4.3 MiB 到 70）。剔除阶段直方图（池 54）：`relevance 29 / stale 16 / author_duplicate 1 / face 1`。
   两期的 `insufficient=True` 均已在 `00-交付说明.md`（"素材不足：True" + 逐条超龄原因）与清单 `warnings` 中写明。
   ⇒ 按用户口径「**准优先，宁可体积不足**」，接受，不为了凑体积放宽闸门。

### 10.5 本轮同时修掉的**验收器械自身缺陷**（教训，别复犯）

| 缺陷 | 现象 | 真因 | 处置 |
|---|---|---|---|
| B4 永不可判 | 4 项"degraded=true 但 warnings 未解释 []" | 器械读 `run_log.json["warnings"]`，**该文件根本没有这个键**（`warnings` 在 `清单.json`）⇒ 恒为空、判据不可伪 | 改读 `清单.json["warnings"]`；实测 pipeline **确实**已写入「题材相关度无法区分本轮候选池：7 个搜索词均未在任何候选标题中命中…」 |
| C5 误报 | 2 项"交付说明已标注=False" | 器械 grep ASCII `insufficient`，而交付说明写的是中文「**素材不足：True**」 | 同时接受两种写法（正则 `素材不足\s*[:：]\s*True`） |
| C4 NameError | 一旦有缺项就崩 | `f"[{mark}]"` 里 `mark` 未定义 | 删掉该占位 |
| C1 幽灵重复 | 跨期去重可能误报全期 `"None"` | 读清单行的 `video_id`，而**旧清单可能没这个键** | 统一按 `04-原片` 文件名反查 aweme_id |

**教训**：验收器械必须先"自证有效"再用于判交付 —— 本轮两个"失败"被误判成交付缺陷，实为器械读错键/读错语言。
**判据写"warnings 已解释"时，必须同时写明 warnings 落在哪个文件的哪个键。**

### 10.6 新增残余发现（**本轮未修，登记下一期**）

**主体词里的通用别名会放行竞品**：Roomba 期的 5 条里有 1 条是 `#科沃斯X12SPRO #科沃斯扫地机器人`，
主题却是「iRobot Roomba 875」。根因是 `_SUBJECT_ALIASES` 把 `扫地机器人` 扩成 `扫地机` 这类**类目词**，
而 `min_subject_hits=1` 只要求命中**任意一个**主体词。
⇒ 下一期方向：把主体词分「**精确词**（品牌/型号）」与「**类目词**」两档，准入要求**至少命中 1 个精确词**
（这与第 4 节"泛词命中不算数"是同一条原则）。**注意**：改了它必须重跑批次，否则交付物与代码版本不对应。

### 10.7 下一期清单（按优先级）

1. `replication_theme.py:364` `expand_keywords` 的 `add(head)` 同族缺陷（造成 `live_count==0` 的**假阴性降级**：充电宝/内存涨价两期 `degraded=true` 但命中率 96% / 38%）。
2. 下载侧多源派发 `CompositeMediaResolver`（`sources` 已实现但**不能启用**：两条链直取 `media_urls[id]`）。
3. 精确词/类目词分档（见 10.6）。
4. 按题材类的时效键，取代单一 `max_age_days=90`。
